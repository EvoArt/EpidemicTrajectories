...
function load_badger_data(dir; brock_changepoint=BROCK_CHANGEPOINT, known_sex_only=true)
    ...
    # REPEAT TESTS. `TestMat.csv` carries MULTIPLE rows for the same
    # (individual, quarter) at 1201 of 12430 cells (9.66%) -- genuine repeat
    # captures within one quarter, not an export artifact: 150 of those cells
    # hold CONTRADICTORY readings for the same test, which mere replication
    # cannot produce.
    #
    # Both reference implementations score EVERY row. Flattening them into one
    # `tests[t,i,j]` slot -- last row wins -- scores each test ONCE and silently
    # drops every repeat reading. The observation term is what separates E from
    # I, so dropping repeat negatives leaves extra I-mass in the filter: it moved
    # tau from ~14.98 to 16.12.
    #
    # `n_neg`/`n_pos` count how many rows read 0 / 1 for each (t, i, j), so each
    # reading is scored once. `tests` keeps the last value for presence checks.
    tests = fill(-1, maxt, m, n_tests)
    n_neg = zeros(Int, maxt, m, n_tests)
    n_pos = zeros(Int, maxt, m, n_tests)
    for r in 1:size(test_mat, 1)
        t = Int(test_mat[r, 1])
        i = Int(test_mat[r, 2])
        ...
        for j in 1:n_tests
            ...
            tests[t, i, j] = Int(v)
            Int(v) == 1 ? (n_pos[t, i, j] += 1) : (n_neg[t, i, j] += 1)
        end
    end
    # The changepoint SWAPS the two Brock columns, so the multiplicities must move
    # with the readings or they desync -- the bug that made the xi sampler inert.
    tests = apply_brock_changepoint!(tests, brock_changepoint)
    n_neg = apply_brock_changepoint!(n_neg, brock_changepoint)
    n_pos = apply_brock_changepoint!(n_pos, brock_changepoint)
...
Where the badger CSVs live. Override with `BADGER_DATA_DIR`.
"""
function DATA_DIR()
    env = get(ENV, "BADGER_DATA_DIR", "")
    isempty(env) || return env
    here = @__DIR__
    for up in (joinpath(here, "..", ".."), joinpath(here, ".."), pwd())
        cand = joinpath(up, "badger_ref", "RData2")
        isdir(cand) && return cand
    end
    error("badger CSVs not found. Set BADGER_DATA_DIR to the directory holding " *
          "dimensions.csv, TestMat.csv, ...")
end

"""
    badger_affected_lists(d)

Who each individual's state affects at each time: its groupmates, excluding
itself. Group membership varies with `t` (badgers move), so this cannot be the
package's fixed-group default.
"""
