# GOMPERTZ-MAKEHAM variant of badger_fit_reststotal_hmc.jl — matches the C++
# reference (badger_ref/classic rcpp) rather than the Julia one (run_base_exp.jl).
#
...
b = badger_data_obssplit(DATA_DIR; trans_mat=badger_transitions_gompertz())
...
function run_badger_fit(n_sweeps; n_burn=0, seed=13)
    ...
    mkpath(outdir)
    x_path = joinpath(outdir, "badger-gompertz-hmc-X-$(n_sweeps)iter.jld2")
...
function save_run(chn, elapsed, n_sweeps; tag)
    ...
    path = joinpath(outdir, "badger-gompertz-hmc-$tag-$stamp.jld2")
    @save path chn elapsed
    timing_path = joinpath(outdir, "badger-gompertz-hmc-$tag-$stamp-timing.txt")
