...
obs_weight(model, data, X, i, t, s) =
    capture_weight(model, data, X, i, t, s) * tests_weight(model, data, X, i, t, s)


# --- reststotal coupling ------------------------------------------------------
# Per-(group,time) running totals of the S->E and S->S moves plus the group
# alive/infected counts, kept consistent with X by four reversible summaries. The
# coupling weight is then a closed form in those totals — O(n_states) per (i,t),
# no neighbour loop.

@inline foi_case(s) = s == I ? 2 : (s == D ? 3 : 1)      # 1=S/E, 2=I, 3=D

@inline function grp_foi(model, data, g, t, c, Iminus, Mminus)   # focal in candidate state
    Iv, Mv = c == 2 ? (Iminus + 1, Mminus + 1) : c == 3 ? (Iminus, Mminus) : (Iminus, Mminus + 1)
    Mv == 0 && return 0.0
    foi = model.lambda * model.alpha[g] + model.beta * Iv / ((Mv / data.K)^model.q)
    -expm1(-foi)
end

function sum_n_infectious(model, data, X, s, i, t, rev=false)
    g = data.social_group[i, t]; g > 0 || return nothing
    c = (s == I)
    rev ? (data.aggregates.n_infectious[g, t] -= c) : (data.aggregates.n_infectious[g, t] += c)
    nothing
end
function sum_n_alive(model, data, X, s, i, t, rev=false)
    g = data.social_group[i, t]; g > 0 || return nothing
    c = (s != D)
    rev ? (data.aggregates.n_alive[g, t] -= c) : (data.aggregates.n_alive[g, t] += c)
    nothing
end
function sum_nSE(model, data, X, s, i, t, rev=false)
    g = data.social_group[i, t]; (g > 0 && t < data.n_timepoints) || return nothing
    c = (s == S) && (X[t + 1, i] == E)
    rev ? (data.aggregates.nSE[g, t] -= c) : (data.aggregates.nSE[g, t] += c)
    nothing
end
function sum_nSS(model, data, X, s, i, t, rev=false)
    g = data.social_group[i, t]; (g > 0 && t < data.n_timepoints) || return nothing
    c = (s == S) && (X[t + 1, i] == S)
    rev ? (data.aggregates.nSS[g, t] -= c) : (data.aggregates.nSS[g, t] += c)
    nothing
end
...
function rest_contribution(model, data, X, i, t, n_states, affected=nothing)
    ...
    Iminus, Mminus = data.aggregates.n_infectious[g, t], data.aggregates.n_alive[g, t]
    nSE, nSS = data.aggregates.nSE[g, t], data.aggregates.nSS[g, t]
    logw = zeros(Float64, n_states)
    @inbounds for s in 1:n_states
        foi = grp_foi(model, data, g, t, foi_case(s), Iminus, Mminus)
...
function build(dir)
    ...
    data = epidemic_data(;
        ...
        derived_summaries=(sum_n_infectious, sum_n_alive, sum_nSE, sum_nSS),
        rest_contribution=rest_contribution,     # the O(n_states) coupling
...
# Explicit starting values near the plausible regime (the diffuse priors make a
# prior-draw start land far out; see badger_run.jl).
...
jld2_path = joinpath(outdir, "badger-reststotal-$stamp.jld2")
csv_path = joinpath(outdir, "badger-reststotal-$stamp.csv")
