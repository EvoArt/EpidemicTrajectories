...
function run_badger_fit(n_sweeps; n_burn=0, seed=13)
    ...
    mkpath(outdir)
    x_path = joinpath(outdir, "badger-gompertz-fixed-hmc-X-$(n_sweeps)iter.jld2")
...
function save_run(chn, elapsed, n_sweeps; tag)
    ...
    path = joinpath(outdir, "badger-gompertz-fixed-hmc-$tag-$stamp.jld2")
    @save path chn elapsed
    timing_path = joinpath(outdir, "badger-gompertz-fixed-hmc-$tag-$stamp-timing.txt")
