...
const N_ADAPT = 200                   # AdaptiveHMC warm-up; learns mass matrix + step size
...
spl = Gibbs(
    (:tau, :alpha, :lambda, :beta, :q, :c1, :a1, :b1, :a2, :b2,
     :thetas, :rhos, :phis, :etas, :nu_cols) => NUTSthenHMC(0.8; n_leapfrog=15),
...
chn = AbstractMCMC.sample(StableRNG(13), m, spl, N_SWEEPS;
                          init=(; X=X0), n_adapts=N_ADAPT, discard_initial=N_ADAPT,
                          adtype=AutoPolyesterForwardDiff(; chunksize=nothing, tag=nothing),save_states=(X=:buffer,))
elapsed = time() - t0
