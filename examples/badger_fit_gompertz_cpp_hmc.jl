# GOMPERTZ-MAKEHAM CPP-PARITY variant — aims for FULL parity with the C++
# reference (badger_ref/classic rcpp), including its statistical QUIRKS.
#
# This is badger_fit_gompertz_fixed_hmc.jl with ONE deliberate change: it does NOT
# CONDITION ON ENTRY. The C++ (logPost_HMC.cpp:59-60) loops the transition
# likelihood from the sampling-period start (not first capture) and separately
# adds the birth->entry cumulative survival term `logS(age_at_entry)`. This pays
# the full probability of surviving birth->entry WITHOUT conditioning on being
# alive at entry — a known quirk that double-counts survival from birth. The
# entry-CONDITIONED (arguably more correct) model is badger_fit_gompertz_fixed_hmc.jl.
#
# All other fixes are shared with gompertz_fixed: mean-time progression, survival
# at t+1, death-ban obs (last capture). Priors match the C++ (tau~Exp(100),
# nu~Dir(1,1,1), no a1/b1).
#
# (original header follows)
# Two deliberate differences from badger_fit_reststotal_hmc.jl, both to match the
# C++ code, everything else identical:
#
#   1. SURVIVAL: Gompertz-Makeham (c1, a2, b2), NOT Siler. The C++ has no a1/b1
#      early-life term at all (grep: a1/b1 appear in zero .cpp files). So a1/b1
#      are DROPPED here — from the model, the HMC block, the step-size vector and
#      init. See badger_model_gompertz.jl.
#
#   2. PRIORS: matched to the C++ (runmodel.R hp_* + logPost_HMC.cpp), which
#      differ from run_base_exp.jl on two parameters:
#        tau: C++ Gamma(1, scale=100) = Exponential(100); run_base_exp uses Exp(10)
#        nu : C++ Dirichlet(1,1,1);    run_base_exp/our default uses Dir(8,1,1)
#      All others already agree: alpha/lambda/beta/c1/a2/b2 ~ Exponential(1),
#      q/thetas/rhos/phis/etas ~ Beta(1,1). (Full audit in the header of the
#      @model below.)
#
...
# double-counted.
# C++ PARITY: NO entry conditioning. The C++ does NOT condition survival on entry
# to the study — its transition likelihood loops from the sampling-period start,
# and it separately ADDS the birth->entry cumulative survival term (below). So
# this loglik uses NO `entry_time` (loops from first_t), and `entry_loglik`
# supplies the pre-study birth->entry survival that the C++ pays. This reproduces
# the reference's statistical choice, INCLUDING its non-conditioning on entry
# (which double-counts survival from birth — a known quirk, see
# cpp_model_discrepancies.md §1). Use badger_fit_gompertz_fixed_hmc.jl instead for
# the entry-CONDITIONED (arguably more correct) model.
loglik = epidemic_loglik(data)                       # NO entry_time — C++ non-conditioning
obs_loglik = epidemic_obs_loglik(data;
                                 observation_process=badger_obs_tests,
                                 observation_weight=badger_obs_tests_weight)
# Birth -> entry cumulative survival (logPost_HMC.cpp:59-60). Only in the C++-parity
# model. See badger_progression_meantime.jl.
entry_loglik = make_entry_survival_loglik()
...
_pars(c) = (; tau=c.values.tau, alpha=c.values.alpha, lambda=c.values.lambda,
            ...
            a2=c.values.a2, b2=c.values.b2, c1=c.values.c1,
...
@model function badger_base(data, n_time, n_ind, n_groups, n_tests, n_seasons, n_nu, loglik_fn, obs_loglik_fn, entry_loglik_fn)
    tau ~ Exponential(100.0)                         # progression scale (C++: Gamma(1,scale=100))
    ...
    c1 ~ Exponential(1.0)                            # Gompertz-Makeham: Makeham constant hazard
    a2 ~ Exponential(1.0)                            # Gompertz late-life scale
    b2 ~ Exponential(1.0)                            # Gompertz late-life rate
    # NO a1/b1: the C++ survival has no early-life term (see badger_model_gompertz.jl).
    ...
    @addlogprob! loglik_fn(pars, data, X) + obs_loglik_fn(pars, data, X) + entry_loglik_fn(pars, data, X)
end
...
function run_badger_fit(n_sweeps; n_burn=0, seed=13)
    m = badger_base(data, data.n_timepoints, data.n_individuals, G, NT, NS, NNU,
                    loglik, obs_loglik, entry_loglik)
    ...
    X0 = copy(raw.X_init)
    # No a1/b1 — Gompertz-Makeham has neither. `badger_initial_params` still
    # returns them (it is shared with the Siler model), so they are simply not
    # copied into `init`.
    init = (; X=X0, tau=init0.tau, alpha=init0.alpha, lambda=init0.lambda,
            beta=init0.beta, q=init0.q, c1=init0.c1,
    ...
    mkpath(outdir)
    x_path = joinpath(outdir, "badger-gompertz-cpp-hmc-X-$(n_sweeps)iter.jld2")
    ...
    println("\nGibbs: fixed-eps HMC(L=$HMC_L, Gompertz-Makeham survival, C++ priors) ",
            "(11 continuous: no a1/b1) + conjugate(etas, nu) + iFFBS(X, reststotal coupling)")
...
function save_run(chn, elapsed, n_sweeps; tag)
    ...
    path = joinpath(outdir, "badger-gompertz-cpp-hmc-$tag-$stamp.jld2")
    @save path chn elapsed
    timing_path = joinpath(outdir, "badger-gompertz-cpp-hmc-$tag-$stamp-timing.txt")
