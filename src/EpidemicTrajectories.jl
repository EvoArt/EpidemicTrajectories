...
# For BatchedArchive only: a stdlib, so it adds no dependency to the four-package
# core. Trajectories go to disc in Int8 batches rather than into memory or a chain
# object, which on a model of any size is the only workable route (residuals.jl).
